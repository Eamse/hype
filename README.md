...
### 04. 배포 후 체감 속도가 느려져서, 원인을 성능 지표로 직접 측정했습니다
...
**해결** 세션에 의존하지 않는 페이지는 `export const revalidate = 60`(ISR)로 전환해 캐싱을 되살림. 단, 로그인 상태에 따라 달라지는 UI(북마크 버튼, 관리자 전용 버튼 등)를 페이지에서 그대로 캐싱하면 다른 유저에게 잘못된 세션 정보가 노출될 위험이 있어, 그 부분만 클라이언트 컴포넌트로 분리해 `useSession()`으로 별도 처리. 개선 효과를 `k6` 부하 테스트로 실측 — `next dev`(p95 37.24s, 2.18 req/s) 대비 프로덕션 빌드(p95 1.97s, 32.8 req/s)로 확인

---

### 05. ISR 전환 후 Vercel 빌드가 `useSearchParams() should be wrapped in a suspense boundary` 에러로 실패했습니다
...
### 06. CDN에서 불러오던 폰트 하나가 페이지 전체 전송량의 대부분을 차지하고 있었습니다
...
**해결** `pretendard` npm 패키지의 가변 폰트(Variable Font, 전체 굵기를 파일 하나로 커버, ~2MB)를 `next/font/local`로 self-host. CDN 왕복이 사라지고, Next.js가 자동으로 preload + `font-display: swap` 처리해줘서 폰트 로딩이 렌더링을 막지 않게 됨
...
**해결** React의 `cache()`로 `product` 조회 함수를 감싸서 한 요청 안에서는 실제 쿼리가 한 번만 나가도록 통합. `product`와 `directorLinks` 조회는 `Promise.all`로 동시에 실행하도록 변경. 실측 결과 캐시 워밍 상태 기준 2.02초 → 1.55초 (약 22% 개선)

---

### 08. 한국에서 접속해도 응답이 느려서 봤더니, 서버리스 함수가 미국에서 실행되고 있었습니다
...
**해결** `vercel.json`에 `{ "regions": ["icn1"] }`를 추가해 서버리스 함수 실행 리전을 서울/인천으로 고정. 자체 서버 전환 전까지, 한국 기반 트래픽(현재 테스트/데모 대상)에 대해 미국 왕복 구간을 제거. 배포 후 `x-vercel-id`가 `icn1::icn1::...`로 바뀐 것을 확인했고, 상품 상세 페이지 응답이 1.55초 → **0.44~0.49초**로 개선(약 70~90%)
