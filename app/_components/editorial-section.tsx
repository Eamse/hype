...
export default function EditorialSection({ magazines, }: {
    magazines: Magazine[];
}) {
    ...
    if (magazines.length === 0)
        return null;
    return (<section className="max-w-[1200px] mx-auto px-5 py-16">
      ...
      <div className="flex items-end justify-between mb-8">
        ...
        <Link href="/editorial" className="text-[11px] tracking-[2px] uppercase text-[black] border-b border-[black] pb-0.5 hover:text-[black] hover:border-[black] transition-colors">
          View All
        </Link>
      </div>
      
      <div ref={gridRef} className="grid grid-cols-2 gap-x-8 gap-y-12">
        {magazines.map((magazine) => (<Link key={magazine.id} href={`/editorial/${magazine.id}`} className="inquiry-step group block">
            
            <div className="relative w-full aspect-square overflow-hidden mb-4 rounded-md">
              {magazine.imageUrl ? (<Image src={magazine.imageUrl} alt={magazine.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105"/>) : (<div className="w-full h-full bg-[white] flex items-center justify-center text-[black] text-xs">
                  No Image
                </div>)}
