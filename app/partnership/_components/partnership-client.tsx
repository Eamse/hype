...
const BOOKING_STEPS = [
    {
        ...
        titleKr: '예약 및 상담',
        descEn: 'Discuss package, shoot type, add-ons, and location — Hype Wedding translates between English and Korean',
...
export default function PartnershipClient() {
    ...
    return (<>
      ...
      <section ref={heroRef} className="partnership-hero">
        <h1 className="partnership-hero-title-en partnership-fade" data-reveal data-reveal-delay="0">
          Business Collaboration
        </h1>
        <p className="partnership-hero-title-kr partnership-fade" data-reveal data-reveal-delay="100">
          비즈니스 협업
        </p>
        <p className="partnership-subcopy-en partnership-fade" data-reveal data-reveal-delay="240">
        ...
        <p className="partnership-subcopy-kr partnership-fade" data-reveal data-reveal-delay="300">
          사진 한 장에 담기는 무한한 이야기. 전 세계 커플과 한국 최고의 작가님들을
          잇습니다. 언어와 거리의 장벽 없이, 처음부터 끝까지.
        </p>
        <div className="partnership-contact partnership-fade" data-reveal data-reveal-delay="440">
      ...
      <section ref={portfolioRef} className="partnership-section">
        ...
        {INFLUENCERS.map((influencer, groupIdx) => (<div key={influencer.name} className="partnership-influencer-group">
            <p className="partnership-influencer-name partnership-fade" data-reveal data-reveal-delay={`${groupIdx * 80}`}>
              {influencer.name}, {influencer.country}
            </p>
            <div className="partnership-photo-row">
              {influencer.photos.map((photo, photoIdx) => (<a key={photo.src} href={influencer.profileUrl} target="_blank" rel="noopener noreferrer" className="partnership-photo partnership-rise" style={{ flex: photo.flex }} data-reveal data-reveal-delay={`${groupIdx * 80 + photoIdx * 120 + 100}`}>
                  <Image src={photo.src} alt={`${influencer.name} photoshoot`} fill className="object-cover"/>
      ...
      <section ref={processRef} className="partnership-section">
        ...
        <div className="partnership-steps-grid">
          {BOOKING_STEPS.map((s, idx) => (<div key={s.step} className="partnership-step-box partnership-rise" data-reveal data-reveal-delay={`${360 + idx * 120}`}>
      ...
      <section ref={pressRef} className="partnership-section">
        ...
        <div className="partnership-logo-grid">
          {PARTNER_LOGOS.map(({ name, logo }, idx) => (<div key={name} className="partnership-logo-box partnership-rise" data-reveal data-reveal-delay={`${240 + idx * 100}`}>
              {logo ? (<Image src={logo} alt={name} width={160} height={160} className="partnership-logo-img"/>) : (name)}
            </div>))}
        </div>
      </section>
      ...
      <section ref={storyRef} className="partnership-section partnership-section--last">
        ...
        <div className="partnership-doc-buttons">
          <a href={BROCHURE_URL} target="_blank" rel="noopener noreferrer" className="partnership-doc-btn partnership-doc-btn--filled partnership-rise" data-reveal data-reveal-delay="240">
          ...
          <a href={BUSINESS_INTRO_URL} target="_blank" rel="noopener noreferrer" className="partnership-doc-btn partnership-doc-btn--outline partnership-rise" data-reveal data-reveal-delay="340">
            <FolderDown size={22} strokeWidth={1.8}/>
            <span>
        ...
        <div className="partnership-contact--footer partnership-fade" data-reveal data-reveal-delay="480">
