...
export default function AdminSignIn() {
    ...
    return (<div style={{
      ...
      <div style={{
            ...
            boxShadow: '0 24px 64px rgba(37,37,37,0.3), inset 0 1px 0 rgba(144,144,144,0.15)',
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <input type="text" value={loginId} placeholder="ID" onChange={(e) => setLoginId(e.target.value)} onFocus={() => setFocusedInput('id')} onBlur={() => setFocusedInput(null)} style={inputStyle(focusedInput === 'id')}/>
          <input type="password" value={password} placeholder="Password" onChange={(e) => setPassword(e.target.value)} onFocus={() => setFocusedInput('pw')} onBlur={() => setFocusedInput(null)} style={inputStyle(focusedInput === 'pw')}/>
          {errors && (<p style={{ fontSize: 12, color: '#999999', margin: '2px 0 0' }}>
              {errors}
            </p>)}
          <button type="submit" onMouseEnter={() => setBtnHover(true)} onMouseLeave={() => setBtnHover(false)} style={{
            marginTop: 10,
            padding: '14px',
            background: '#fff',
            color: '#000',
