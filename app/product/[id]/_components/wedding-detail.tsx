...
function InstagramLink({ handles }: { handles: string[] }) {
  if (handles.length === 0) return null;
  return (
    <span className="partners-grid-item-ig">
      {handles.map((h, i) => (
        <span key={h}>
          <a
            ...
            className="partners-grid-item-ig-link"
          >
            {h}
          </a>
          {i < handles.length - 1 && ' / '}
...
export default function WeddingDetail({
  ...
  const inclusions = allInclusions.filter(
    ({ inclusion }) => !inclusion.name.trim().startsWith('Note:'),
  );
  const inclusionNotes = allInclusions.filter(({ inclusion }) =>
    inclusion.name.trim().startsWith('Note:'),
  );
  ...
  const partnerRowsRaw = [
    {
      role: 'Photographer',
      name: title,
      instagramHandles: (activePackage?.director.instagram ?? '')
        .split(' / ')
        .map((h) => h.trim())
        .filter(Boolean),
    },
  ...
  const partnerRows = Object.values(
    partnerRowsRaw.reduce(
      (acc, row) => {
        const existing = acc[row.role];
        if (existing) {
          existing.name = [
            ...new Set([...existing.name.split(' / '), row.name]),
          ].join(' / ');
          existing.instagramHandles = [
            ...existing.instagramHandles,
            ...row.instagramHandles,
          ];
        } else {
          acc[row.role] = {
            ...row,
            instagramHandles: [...row.instagramHandles],
          };
        }
        return acc;
      },
      {} as Record<
        string,
        { role: string; name: string; instagramHandles: string[] }
      >,
    ),
  );
  const shootDetails = activePackage
    ? [
        {
          label: 'Duration',
          value: activePackage.shootingTime,
          detail: activePackage.shootingTimeDetail,
        },
        {
          label: 'Locations',
          value: activePackage.locations,
          detail: activePackage.locationsDetail,
        },
        {
      ...
      <div className={`bg-white border ${BORDER} rounded-2xl overflow-hidden`}>
          ...
          {directors.length > 1 && (
            <div className="flex gap-[6px] justify-center flex-wrap mb-2">
              {directors.map((d) => (
                <span
                  key={d.id}
                  className={`text-[12px] py-[5px] px-[14px] border rounded-[20px] cursor-pointer ${tabState(activeDirectorId === d.id)}`}
          ...
          {hasDetails && activePackage && (
            <>
              ...
              <div className="grid grid-cols-2 gap-y-4 min-[480px]:grid-cols-4 min-[480px]:gap-y-0">
                {shootDetails.map((d) => {
                  const { num, unit: parsedUnit } = parseShootDetailValue(
                    d.value,
                  );
                  const unit =
                    parsedUnit ||
                    (d.label === 'Original Photos' ? 'photos' : '');
                  return (
                    <div
                      key={d.label}
        ...
        {activePackage && priceCount > 0 && session && (
          <div className={`border-t ${BORDER}`}>
            ...
            >
              {activePackage.isSinglePrice ? (
                <div className="py-4 px-3 sm:py-5 sm:px-6 flex flex-col gap-1 bg-[#2d5a45] border-r border-[#EEEEEE]">
                ...
                <>
                  {activePackage.hasPriceSNS && (
                    <div
                      className={`py-4 px-3 sm:py-5 sm:px-6 flex flex-col gap-1 bg-[#2d5a45] ${priceCount > 1 ? `border-r ${BORDER}` : ''}`}
                    >
                      <div className="flex flex-col items-center gap-[2px] text-center">
                        <span
