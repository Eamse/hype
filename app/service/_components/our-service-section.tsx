...
export default function OurServiceSection({ showHeader = true, }: {
    showHeader?: boolean;
}) {
    ...
    return (<div ref={rootRef} className="offer-page">
      <div className="offer-header">
        ...
        {!showHeader && (<h2 data-offer-reveal className="offer-headline offer-fade">
            Our Service
          </h2>)}
      </div>
      ...
      <div className="service-grid">
        {SERVICES.map((service, idx) => {
            ...
            return (<div key={service.title} className="service-col">
              {idx > 0 && (<span data-offer-reveal data-offer-reveal-delay={Math.max(base - 50, 0)} className="service-divider offer-fade"/>)}
              <h3 data-offer-reveal data-offer-reveal-delay={base + TITLE_OFFSET} className="service-col-title offer-rise">
                {service.title}
              </h3>
              <p data-offer-reveal data-offer-reveal-delay={base + TAGLINE_OFFSET} className="service-col-tagline offer-fade">
                &quot;{service.tagline}&quot;
              </p>
              <ul data-offer-reveal data-offer-reveal-delay={base + LIST_OFFSET} className={`service-col-list offer-fade${service.bulleted ? ' bulleted' : ''}`}>
                {service.items.map((item) => (<li key={item.text}>
                    {item.text.startsWith('H&MU') ? (<>
                        <u>H&MU</u>
                        {item.text.slice('H&MU'.length)}
                      </>) : (item.text)}
                    {item.note && (<div className="service-col-note">{item.note}</div>)}
                  </li>))}
              </ul>
              {service.footnote && (<p data-offer-reveal data-offer-reveal-delay={base + LIST_OFFSET + 100} className="service-col-note offer-fade" style={{ marginTop: 5 }}>
                  {service.footnote}
                </p>)}
            </div>);
        })}
