...
export default function MagazineManageList({ initialMagazines, }: {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <BulkActions total={magazines.length} selectedCount={selectedIds.size} allSelected={selectedIds.size === magazines.length && magazines.length > 0} onToggleAll={toggleAll} onDeleteSelected={handleBulkDelete}/>
      {magazines.map((m) => (<div key={m.id} style={{
                display: 'flex',
                alignItems: 'center',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
            <input type="checkbox" checked={selectedIds.has(m.id)} onChange={() => toggleSelect(m.id)} style={{ flexShrink: 0 }}/>
            <span style={{
                fontSize: 11,
                fontWeight: 700,
                padding: '2px 8px',
          ...
          <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
            <button onClick={() => handleTogglePin(m.id, m.isPinned)} disabled={pinningId === m.id} style={{
                padding: '6px 12px',
                borderRadius: 6,
                border: '1px solid #000',
                fontSize: 12,
            ...
            <Link href={`/editorial/edit/${m.id}`} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #000', fontSize: 12, color: '#000', textDecoration: 'none' }}>
              Edit
            </Link>
            <button onClick={() => handleDelete(m.id)} disabled={deletingId === m.id} style={{ padding: '6px 12px', borderRadius: 6, border: 'none', background: 'none', color: '#ef4444', fontSize: 12, cursor: 'pointer' }}>
              Delete
            </button>
          </div>
        </div>))}
