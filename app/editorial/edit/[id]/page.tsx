...
export default async function MagazineEditPage({ params }: Props) {
    ...
    return (<div style={{ minHeight: '100vh' }}>
      <Header brand="hype-wedding"/>
      <MagazineWriteForm magazineId={magazine.id} initialTitle={magazine.title} initialContent={magazine.content} initialImageUrl={magazine.imageUrl} initialImages={magazine.images.map((img) => ({ id: img.id, url: img.url }))} initialPublished={magazine.published}/>
    </div>);
}
