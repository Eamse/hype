...
export default function MagazineWriteForm({ magazineId, initialTitle = '', initialContent = '', initialImageUrl = null, initialImages = [], initialPublished = true, }: {
    magazineId?: number;
    initialTitle?: string;
    ...
    async function handleDeleteExistingImage(imageId: number) {
        ...
        try {
            const res = await fetch(`/editorial/${magazineId}/images?imageId=${imageId}`, {
                method: 'DELETE',
            });
            if (res.ok) {
    ...
    return (<div style={{ maxWidth: 640, margin: '0 auto', padding: '80px 20px 80px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div>
          ...
          {existingImageUrl && !coverFile && (<p style={{ fontSize: 12, color: '#000', margin: '0 0 6px' }}>
        ...
        <div>
          ...
          {existingImages.length > 0 && (<div style={{
                display: 'flex',
                flexWrap: 'wrap',
                gap: 8,
                marginBottom: 10,
              ...
              {existingImages.map((img) => (<div key={img.id} style={{ position: 'relative' }}>
                  ...
                  <button type="button" onClick={() => handleDeleteExistingImage(img.id)} disabled={deletingImageId === img.id} style={{
                    position: 'absolute',
                    top: -6,
                    right: -6,
                    width: 20,
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button type="button" onClick={() => setPreview(true)} disabled={!title.trim() && !content.trim()} style={{
            padding: '12px 20px',
            borderRadius: 8,
            border: '1px solid #000',
            background: '#fff',
          ...
          <button type="button" onClick={handleSave} disabled={submitting} style={{
            padding: '12px 20px',
            borderRadius: 8,
            border: 'none',
            background: '#000',
