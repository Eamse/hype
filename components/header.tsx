...
function DropdownLink({ href, label, indent, }: {
    href: string;
    label: string;
    indent?: boolean;
}) {
    const [hovered, setHovered] = useState(false);
    return (<Link href={href} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{
            display: 'block',
            padding: '14px 22px',
            ...
            transition: 'background-color 0.15s, font-size 0.15s, border-color 0.15s',
...
function HamburgerIcon() {
    return (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round">
...
export default function Header(props: {
    ...
    return (<Suspense fallback={<div style={{ height: 56 }}/>}>
      <HeaderInner {...props}/>
    </Suspense>);
}
function HeaderInner({ brand = 'hype-wedding' }: {
    brand?: Brand;
}) {
    ...
    return (<>
      <header style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {(Object.keys(BRANDS) as Brand[]).map((b, i) => (<div key={b} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {!isMobile &&
            (() => {
                const allDropdownGroups = NAV_LINKS[brand].filter((i) => i.dropdown);
                return (<div onMouseLeave={closeNav}>
                  <nav style={{ display: 'flex', gap: 32 }}>
                    {NAV_LINKS[brand].map(({ label, href, dropdown }) => {
                        const active = href !== '#' && pathname === href;
                        if (dropdown) {
                            return (<div key={label} ref={(el) => {
                                    if (el)
                                        navItemRefs.current.set(label, el);
                                }} onMouseEnter={() => openNav(label)} style={{ display: 'flex', alignItems: 'center' }}>
                            <Link href={href} style={{
                              ...
                              {(hoveredNav !== null
                                    ...
                                    : active) && (<span style={{
                                        ...
                                        right: 0,
                                        height: 1.5,
                                        background: hoveredNav === label ? '#2D5A45' : '#000',
                                    }}/>)}
                            </Link>
                          </div>);
                        }
                        return (<Link key={label} href={href} onMouseEnter={() => openNav(label)} style={{
                                fontSize: 14,
                                fontWeight: active ? 600 : 'bold',
                                color: hoveredNav === label ? '#2D5A45' : '#000',
                                position: 'relative',
                  ...
                  {hoveredNav !== null &&
                        allDropdownGroups
                            ...
                            return (<div key={group.label} className="header-dropdown" onMouseEnter={() => {
                                    if (closeTimer.current)
                                        clearTimeout(closeTimer.current);
                                }} onMouseLeave={closeNav} style={{
                                    position: 'fixed',
                                    top: 47,
                                    left,
                                    width: 'fit-content',
                            ...
                            {group.dropdown!.map(({ label: dLabel, href: dHref, indent }, i) => (<div key={dLabel} style={{
                                        borderTop: i === 0 ? 'none' : '1px solid #eee',
                                    }}>
                                  <DropdownLink href={dHref} label={dLabel} indent={indent}/>
                                </div>))}
                          </div>);
                        })}
                </div>);
          ...
          <div style={{
            ...
            {!isMobile ? (<HeaderActionButtons session={session} showBell notifOpen={notifOpen} onNotifToggle={handleNotifToggle} onNotifClose={() => setNotifOpen(false)} onSearchClick={() => setSearchOpen(true)} onSignClick={handleSignClick}/>) : (<button onClick={() => setMenuOpen((v) => !v)} aria-label={menuOpen ? 'Close menu' : 'Open menu'}>
                {menuOpen ? <CloseIcon /> : <HamburgerIcon />}
              </button>)}
            {profileMenuOpen && !isMobile && (<div style={{
                position: 'absolute',
                top: '100%',
                right: 0,
                marginTop: 12,
      ...
      {isMobile && menuOpen && (<div className="mobile-drawer" style={{
                position: 'fixed',
                top: 56,
                left: 0,
                right: 0,
          ...
          <div style={{
            ...
            <HeaderActionButtons session={session} showBell showSns={false} notifOpen={notifOpen} onNotifToggle={handleNotifToggle} onNotifClose={() => setNotifOpen(false)} onSearchClick={() => {
                setMenuOpen(false);
                setSearchOpen(true);
            }} onSignClick={() => {
                if (session)
          ...
          <nav style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {NAV_LINKS[brand].map(({ label, href, dropdown }) => {
                ...
                if (dropdown) {
                    return (<div key={label} style={{ borderBottom: '1px solid #000' }}>
                    <button onClick={() => setOpenMobileDropdown(isOpen ? null : label)} style={{
                            width: '100%',
                            display: 'flex',
                            justifyContent: 'space-between',
                            alignItems: 'center',
                      ...
                      {label}
                      <span className={`mobile-drawer-toggle-icon${isOpen ? ' open' : ''}`} style={{ fontSize: 16, color: '#000' }}>
                        +
                      </span>
                    </button>
                    {isOpen && (<div className="mobile-subdrawer" style={{
                                display: 'flex',
                                flexDirection: 'column',
                                gap: 0,
                                paddingBottom: 12,
                            }}>
                        {dropdown.map(({ label: dLabel, href: dHref }) => (<Link key={dLabel} href={dHref} onClick={() => setOpenMobileDropdown(null)} style={{
                                    fontSize: 15,
                                    color: '#000',
                ...
                return (<Link key={label} href={href} style={{
                        fontSize: 22,
                        fontWeight: active ? 700 : 400,
                        color: '#000',
                        padding: '16px 0',
      ...
      {isMobile && !menuOpen && (<nav style={{
          ...
          {([
                ...
                return 'href' in item ? (<Link key={item.label} href={item.href} style={style}>
                {content}
              </Link>) : (<button key={item.label} onClick={item.onClick} style={style}>
                {content}
              </button>);
            })}
        </nav>)}
