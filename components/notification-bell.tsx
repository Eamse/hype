...
export default function NotificationBell({ open, onToggle, onClose, }: {
    open: boolean;
    onToggle: () => void;
    onClose: () => void;
}) {
    ...
    return (<div ref={containerRef} style={{ position: 'relative' }}>
      <button onClick={onToggle} aria-label={count > 0 ? `Notifications (${count} unread)` : 'Notifications'} style={{
            position: 'relative',
            display: 'flex',
            alignItems: 'center',
      ...
      {open && (<div style={{
          ...
          <div style={{
            ...
            {count > 0 && (<button onClick={handleMarkAllRead} style={{
                    background: 'none',
                    border: 'none',
                    cursor: 'pointer',
                    fontSize: 12,
          ...
          {count === 0 ? (<p style={{
            ...
            </p>) : (notifications.map((n) => (<button key={n.id} onClick={() => handleClickItem(n)} style={{
                    display: 'block',
                    width: '100%',
                    textAlign: 'left',
                    padding: '12px 16px',
