...
const SECTION_COLOR: Record<SearchResult['type'], {
    bg: string;
    text: string;
}> = {
...
function ResultThumb({ item }: {
    ...
    return (<div style={{
      ...
      {item.imageUrl && (<Image src={item.imageUrl} alt={item.title} fill sizes="52px" style={{ objectFit: 'cover', opacity: loaded ? 1 : 0, transition: 'opacity 0.25s ease' }} onLoad={() => setLoaded(true)}/>)}
    </div>);
}
      ...
      <div style={{
        ...
        <div style={{ padding: '18px 20px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{
            ...
            <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search products, photographers, reviews, editorial" style={{
            flex: 1,
            fontSize: 15,
            border: 'none',
            outline: 'none',
            ...
            {query && (<button onClick={() => setQuery('')} aria-label="Clear" style={{
                fontSize: 12,
                color: '#aaa',
                background: '#e4e4e4',
                border: 'none',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 20px' }}>
          ...
          {loading && (<p style={{ fontSize: 13, color: '#999', padding: '20px 0' }}>Searching...</p>)}
          {!loading && query && results.length === 0 && (<div style={{
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'center',
                gap: 6,
          ...
          {!loading &&
            groups.map((group) => (<div key={group.type} style={{ marginTop: 18 }}>
                <SectionBadge type={group.type}/>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {group.items.map((item) => (<div key={`${item.type}-${item.id}`} onClick={() => {
                        router.push(detailHref(item));
                        onClose();
                    }} className="search-result-row" style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: 12,
                        padding: '8px 8px',
