...
import { LOCATIONS, locationOfSection, pkgFormFromPackage, emptyDirForm, emptyPkgForm, type Director, type Package, type PkgForm, type SimpleProduct, type Inclusion, type Addon, type Partner, } from './wedding-photographer-types';
export default function WeddingPhotographerPanel() {
    ...
    async function handleExpandDir(dirId: number) {
        ...
        setEditPkgId(null);
        if (!dirPackages[dirId])
            await loadPackages(dirId);
    }
            ...
            for (const pkgForm of newDirPkgForms) {
                ...
                const pkgRes = await fetch('/api/admin/packages', {
                    ...
                    body: JSON.stringify({
                        ...
                        isSinglePrice: pkgForm.isSinglePrice,
                        shootingTime: pkgForm.shootingTime,
                        shootingTimeDetail: pkgForm.shootingTimeDetail || null,
                        locations: pkgForm.locations,
                        locationsDetail: pkgForm.locationsDetail || null,
    ...
    async function handleBulkDeleteDirs() {
        await Promise.all([...selectedIds].map((id) => fetch(`/api/admin/wedding-photographers/${id}`, { method: 'DELETE' })));
        await loadDirectors();
        clearSelection();
    }
    async function handleDeleteDir(id: number) {
        if (!confirm('이 작가를 삭제하면 연결된 패키지도 모두 삭제됩니다. 계속할까요?'))
            return;
        const res = await fetch(`/api/admin/wedding-photographers/${id}`, {
            method: 'DELETE',
        });
    ...
    async function savePkgRelations(pkgId: number, form: PkgForm) {
        const partnerIds = [...form.videographerIds, ...form.hmuIds, ...form.dressIds, ...form.suitIds, ...form.bouquetIds];
        await Promise.all([
            ...
            const res = await fetch('/api/admin/packages', {
                ...
                body: JSON.stringify({
                    ...
                    priceNoSNS: Number(newPkgForm.priceNoSNS) || 0,
                        isSinglePrice: newPkgForm.isSinglePrice,
                    shootingTime: newPkgForm.shootingTime,
                    shootingTimeDetail: newPkgForm.shootingTimeDetail || null,
                    locations: newPkgForm.locations,
                    locationsDetail: newPkgForm.locationsDetail || null,
            ...
            const res = await fetch(`/api/admin/packages/${pkgId}`, {
                ...
                body: JSON.stringify({
                    ...
                    priceNoSNS: Number(editPkgForm.priceNoSNS) || 0,
                        isSinglePrice: editPkgForm.isSinglePrice,
                    shootingTime: editPkgForm.shootingTime,
                    shootingTimeDetail: editPkgForm.shootingTimeDetail || null,
                    locations: editPkgForm.locations,
                    locationsDetail: editPkgForm.locationsDetail || null,
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <input style={inputStyle} placeholder="#1-1" value={dirForm.number} onChange={(e) => setDirForm((f) => ({ ...f, number: e.target.value }))}/>
          </div>
          <div>
            <label style={labelStyle}>지역 *</label>
            <select style={inputStyle} value={dirForm.location} onChange={(e) => setDirForm((f) => ({
            ...f,
          ...
          <div>
            ...
            <select style={inputStyle} value={dirForm.productId} disabled={!dirForm.location} onChange={(e) => setDirForm((f) => ({
            ...f,
          ...
          <div>
            ...
            <input style={inputStyle} placeholder="Jeju and You" value={dirForm.name} onChange={(e) => setDirForm((f) => ({ ...f, name: e.target.value }))}/>
          </div>
          <div>
            <label style={labelStyle}>인스타그램</label>
            <input style={inputStyle} placeholder="@jejuandyou_snap" value={dirForm.instagram} onChange={(e) => setDirForm((f) => ({ ...f, instagram: e.target.value }))}/>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            textTransform: 'uppercase',
              ...
              Packages{' '}
              {newDirPkgForms.length > 0 && (<span style={{ color: '#acacac' }}>
                  ({newDirPkgForms.length}개)
                </span>)}
            </p>
            <button style={btnStyle('transparent', '#252525', '#000')} onClick={() => setNewDirPkgForms((prev) => [...prev, { ...emptyPkgForm }])}>
              + 패키지 추가
            </button>
          </div>

          {newDirPkgForms.map((pkgForm, idx) => (<div key={idx} style={{ marginBottom: 12 }}>
              <div style={{
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'space-between',
              ...
              <PackageForm form={pkgForm} onChange={(f) => setNewDirPkgForms((prev) => prev.map((p, i) => (i === idx ? f : p)))} allInclusions={allInclusions} allAddons={allAddons} allPartners={allPartners} onSave={() => { }} onCancel={() => { }} saving={false} hideButtons/>
            </div>))}
        </div>

        <button style={btnStyle('#000', '#fff')} onClick={handleCreateDir} disabled={saving}>
          {saving ? '등록 중...' : '등록'}
        </button>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {!loading && directors.length === 0 && (<p style={{ fontSize: 13, color: '#000' }}>등록된 작가가 없습니다.</p>)}
        <BulkActions total={directors.length} selectedCount={selectedIds.size} allSelected={selectedIds.size === directors.length && directors.length > 0} onToggleAll={toggleAll} onDeleteSelected={handleBulkDeleteDirs}/>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          {groupedDirectors.map(({ label, directors: group }) => (<div key={label} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <p style={{
                fontSize: 12,
                fontWeight: 700,
                color: '#5a5a5a',
              ...
              {group.map((dir) => (<DirectorRow key={dir.id} dir={dir} isExpanded={expandedDirId === dir.id} packages={dirPackages[dir.id] ?? []} isSelected={selectedIds.has(dir.id)} onToggleSelect={() => toggleSelect(dir.id)} onToggleExpand={() => handleExpandDir(dir.id)} isEditingDir={editDirId === dir.id} editDirForm={editDirForm} onEditDirFormChange={setEditDirForm} onStartEditDir={() => {
                    setEditDirId(dir.id);
                    setEditDirForm({
                        number: dir.number,
                        name: dir.name,
                        instagram: dir.instagram ?? '',
                        location: (dir.location as '' | 'Jeju' | 'Seoul' | null) ?? '',
                        productId: dir.products[0]?.productId ?? '',
                    });
                    if (!dirPackages[dir.id])
                        loadPackages(dir.id);
                }} onSaveEditDir={() => handleUpdateDir(dir.id, dir.products[0]?.productId)} onCancelEditDir={() => setEditDirId(null)} onDeleteDir={() => handleDeleteDir(dir.id)} allProducts={allProducts} pkgsLoading={pkgsLoading} isAddingPkg={addingPkgForDir === dir.id} newPkgForm={newPkgForm} onNewPkgFormChange={setNewPkgForm} onStartAddPkg={() => {
                    setAddingPkgForDir(dir.id);
                    setNewPkgForm(emptyPkgForm);
                    setEditPkgId(null);
                }} onCancelAddPkg={() => setAddingPkgForDir(null)} onSaveNewPkg={() => handleCreatePkg(dir.id)} editPkgId={editPkgId} editPkgForm={editPkgForm} onEditPkgFormChange={setEditPkgForm} onStartEditPkg={(pkg) => {
                    setEditPkgId(pkg.id);
                    setEditPkgForm(pkgFormFromPackage(pkg));
                    setAddingPkgForDir(null);
                }} onCancelEditPkg={() => setEditPkgId(null)} onSaveEditPkg={(pkgId) => handleUpdatePkg(pkgId, dir.id)} onDeletePkg={(pkgId) => handleDeletePkg(pkgId, dir.id)} allInclusions={allInclusions} allAddons={allAddons} allPartners={allPartners} saving={saving}/>))}
            </div>))}
        </div>
      </div>
    </div>);
