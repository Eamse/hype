...
type Director = {
  ...
  order: number;
  products: { productId: number; product: { id: number; title: string } }[];
};
...
type Partner = { id: number; role: string; name: string; instagram: string | null };
...
const emptyDirForm = { number: '', name: '', instagram: '', productId: '' as number | '' };
...
function PackageForm({
  ...
  const sectionTitle: React.CSSProperties = {
    fontSize: 10, fontWeight: 700, color: '#c9a96e', letterSpacing: '1.5px',
    textTransform: 'uppercase', marginBottom: 8, marginTop: 16,
  };

  return (
    <div style={{ background: '#fdfcfa', border: '1px solid #e8e0d0', borderRadius: 10, padding: 20 }}>
      {/* 기본 정보 */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div>
          <label style={labelStyle}>패키지명 *</label>
          <input style={inputStyle} value={form.name} placeholder="Package A"
            onChange={(e) => onChange({ ...form, name: e.target.value })} />
        </div>
        <div>
          <label style={labelStyle}>서브타이틀</label>
          <input style={inputStyle} value={form.subtitle} placeholder="Jeju and You Main & Nervi"
            onChange={(e) => onChange({ ...form, subtitle: e.target.value })} />
        </div>
        <div>
          <label style={labelStyle}>SNS 동의 가격 (USD)</label>
          <input style={inputStyle} type="number" value={form.priceSNS} placeholder="2690"
            onChange={(e) => onChange({ ...form, priceSNS: e.target.value })} />
        </div>
        <div>
          <label style={labelStyle}>SNS 비동의 가격 (USD)</label>
          <input style={inputStyle} type="number" value={form.priceNoSNS} placeholder="2830"
            onChange={(e) => onChange({ ...form, priceNoSNS: e.target.value })} />
        </div>
        <div>
          <label style={labelStyle}>촬영 시간</label>
          <input style={inputStyle} value={form.shootingTime} placeholder="4 hours"
            onChange={(e) => onChange({ ...form, shootingTime: e.target.value })} />
        </div>
        <div>
          <label style={labelStyle}>장소 수</label>
          <input style={inputStyle} value={form.locations} placeholder="3 sites"
            onChange={(e) => onChange({ ...form, locations: e.target.value })} />
        </div>
        <div>
          <label style={labelStyle}>원본 사진 수</label>
          <input style={inputStyle} value={form.originalPhotos} placeholder="800+"
            onChange={(e) => onChange({ ...form, originalPhotos: e.target.value })} />
        </div>
        <div>
          <label style={labelStyle}>보정본 수</label>
          <input style={inputStyle} type="number" value={form.retouched} placeholder="40"
            onChange={(e) => onChange({ ...form, retouched: e.target.value })} />
        </div>
        <div style={{ gridColumn: '1 / -1' }}>
          <label style={labelStyle}>보정 상세</label>
          <input style={inputStyle} value={form.retouchedDetail}
            placeholder="Detailed Retouched: 25 (Customer Selected) + Color Correction: 15"
            onChange={(e) => onChange({ ...form, retouchedDetail: e.target.value })} />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
        <div>
          ...
          <select style={inputStyle} value={form.hmuId ?? ''}
            onChange={(e) => onChange({ ...form, hmuId: e.target.value ? Number(e.target.value) : null })}>
            <option value="">없음</option>
            {hmuList.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </div>
        <div>
          <label style={labelStyle}>Dress</label>
          <select style={inputStyle} value={form.dressId ?? ''}
            onChange={(e) => onChange({ ...form, dressId: e.target.value ? Number(e.target.value) : null })}>
            <option value="">없음</option>
            {dressList.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </div>
        <div>
          <label style={labelStyle}>Suit</label>
          <select style={inputStyle} value={form.suitId ?? ''}
            onChange={(e) => onChange({ ...form, suitId: e.target.value ? Number(e.target.value) : null })}>
            <option value="">없음</option>
            {suitList.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </div>
      </div>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 6 }}>
            {allInclusions.map((inc) => (
              <label key={inc.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer' }}>
                <input type="checkbox" checked={form.inclusionIds.includes(inc.id)}
                  onChange={() => onChange({ ...form, inclusionIds: toggleId(form.inclusionIds, inc.id) })} />
                {inc.name}
              </label>
            ))}
          </div>
      ...
      {allAddons.length > 0 && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 6 }}>
            {allAddons.map((addon) => (
              <label key={addon.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer' }}>
                <input type="checkbox" checked={form.addonIds.includes(addon.id)}
                  onChange={() => onChange({ ...form, addonIds: toggleId(form.addonIds, addon.id) })} />
                {addon.name} <span style={{ color: '#c9a96e', fontSize: 12 }}>${addon.price}</span>
              </label>
            ))}
          </div>
        </>
      ...
      {!hideButtons && (
        <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
          <button style={btnStyle('#191919', '#fff')} onClick={onSave} disabled={saving}>
            {saving ? '저장 중...' : '저장'}
          </button>
          <button style={btnStyle('#fff', '#666', '#ddd')} onClick={onCancel}>취소</button>
        </div>
      )}
    </div>
  );
...
export default function WeddingPhotographerPanel() {
  ...
  const { selectedIds, toggleSelect, toggleAll, clearSelection } = useSelection(directors);

  async function loadDirectors() {
    const data = await fetch('/api/admin/wedding-photographers').then((r) => r.json());
  ...
  async function loadPackages(directorId: number) {
    setPkgsLoading(true);
    try {
      const data = await fetch(`/api/admin/packages?directorId=${directorId}`).then((r) => r.json());
      setDirPackages((prev) => ({ ...prev, [directorId]: Array.isArray(data) ? data : [] }));
    } finally {
  ...
  useEffect(() => {
    Promise.all([
      loadDirectors(),
      fetch('/api/admin/inclusions').then((r) => r.json()).then((d) => setAllInclusions(Array.isArray(d) ? d : [])),
      fetch('/api/admin/addons').then((r) => r.json()).then((d) => setAllAddons(Array.isArray(d) ? d : [])),
      fetch('/api/admin/partners').then((r) => r.json()).then((d) => setAllPartners(Array.isArray(d) ? d : [])),
  ...
  async function handleUpdateDir(id: number, oldProductId?: number) {
    ...
    if (newProductId !== (oldProductId ?? '')) {
      if (oldProductId) {
        await fetch(`/api/admin/wedding-directors/${oldProductId}/${id}`, { method: 'DELETE' });
      }
      if (newProductId) {
  ...
  async function handleDeleteDir(id: number) {
    if (!confirm('이 작가를 삭제하면 연결된 패키지도 모두 삭제됩니다. 계속할까요?')) return;
    const res = await fetch(`/api/admin/wedding-photographers/${id}`, { method: 'DELETE' });
    if (!res.ok) { alert((await res.json()).error); return; }
  ...
  async function savePkgRelations(pkgId: number, form: PkgForm) {
    const partnerIds = [form.hmuId, form.dressId, form.suitId].filter((id): id is number => id !== null);
    await Promise.all([
  ...
  async function handleCreatePkg(directorId: number) {
    if (!newPkgForm.name.trim()) { alert('패키지명은 필수입니다.'); return; }
    setSaving(true);
    try {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      ...
      <div style={{ paddingBottom: 20, borderBottom: '1px solid #ede8de' }}>
        <p style={{ fontSize: 10, letterSpacing: '2px', color: '#7a5520', fontWeight: 600, marginBottom: 6 }}>MANAGE</p>
        <h2 style={{ fontSize: 22, fontWeight: 700, color: '#1a1a1a' }}>Directors 관리</h2>
        <p style={{ fontSize: 12, color: '#999', marginTop: 4 }}>작가 정보와 패키지(가격·구성·파트너)를 여기서 모두 관리합니다.</p>
      </div>

      {/* 작가 등록 폼 */}
      <div style={{ background: '#fdfcfa', border: '1px solid #ede8de', borderRadius: 12, padding: 24 }}>
        <p style={{ fontSize: 13, fontWeight: 600, color: '#3a1a2a', marginBottom: 16 }}>새 작가 등록</p>

        {/* 기본 정보 */}
        <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
          <div>
            <label style={labelStyle}>번호 *</label>
            <input style={inputStyle} placeholder="#1-1" value={dirForm.number}
              onChange={(e) => setDirForm((f) => ({ ...f, number: e.target.value }))} />
          </div>
          <div>
            <label style={labelStyle}>상품</label>
            <select style={inputStyle} value={dirForm.productId}
              onChange={(e) => setDirForm((f) => ({ ...f, productId: e.target.value ? Number(e.target.value) : '' }))}>
              <option value="">없음</option>
          ...
          <div>
            ...
            <input style={inputStyle} placeholder="Jeju and You" value={dirForm.name}
              onChange={(e) => setDirForm((f) => ({ ...f, name: e.target.value }))} />
          </div>
          <div>
            <label style={labelStyle}>인스타그램</label>
            <input style={inputStyle} placeholder="@jejuandyou_snap" value={dirForm.instagram}
              onChange={(e) => setDirForm((f) => ({ ...f, instagram: e.target.value }))} />
          </div>
        </div>

        {/* 패키지 */}
        <div style={{ borderTop: '1px solid #ede8de', paddingTop: 16, marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: '#3a1a2a', letterSpacing: '1px', textTransform: 'uppercase' }}>
              Packages {newDirPkgForms.length > 0 && <span style={{ color: '#c9a96e' }}>({newDirPkgForms.length}개)</span>}
            </p>
            <button style={btnStyle('transparent', '#3a1a2a', '#ddd')} onClick={() =>
          ...
          {newDirPkgForms.map((pkgForm, idx) => (
            <div key={idx} style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <p style={{ fontSize: 12, fontWeight: 600, color: '#666' }}>Package {idx + 1}</p>
                <button
                  style={{ background: 'none', border: 'none', fontSize: 12, color: '#e05555', cursor: 'pointer' }}
                  onClick={() => setNewDirPkgForms((prev) => prev.filter((_, i) => i !== idx))}
                >
                  제거
                </button>
              </div>
              <PackageForm
                form={pkgForm}
                onChange={(f) => setNewDirPkgForms((prev) => prev.map((p, i) => i === idx ? f : p))}
        ...
        <button style={btnStyle('#191919', '#fff')} onClick={handleCreateDir} disabled={saving}>
          {saving ? '등록 중...' : '등록'}
        </button>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <BulkActions
          ...
          allSelected={selectedIds.size === directors.length && directors.length > 0}
        ...
        {directors.map((dir) => {
          ...
          return (
            <div key={dir.id} style={{ background: '#fff', border: '1px solid #ede8de', borderRadius: 12, overflow: 'hidden' }}>
              {/* 작가 행 */}
              <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 10 }}>
                <input type="checkbox" checked={selectedIds.has(dir.id)}
                  onClick={(e) => e.stopPropagation()} onChange={() => toggleSelect(dir.id)}
                  style={{ flexShrink: 0 }} />
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                {editDirId === dir.id ? (
                  <div style={{ display: 'flex', gap: 10, flex: 1, flexWrap: 'wrap', alignItems: 'center' }}>
                    <input style={{ ...inputStyle, width: 80 }} value={editDirForm.number}
                      onChange={(e) => setEditDirForm((f) => ({ ...f, number: e.target.value }))} placeholder="번호" />
                    <select style={{ ...inputStyle, minWidth: 120 }} value={editDirForm.productId}
                      onChange={(e) => setEditDirForm((f) => ({ ...f, productId: e.target.value ? Number(e.target.value) : '' }))}>
                      <option value="">상품 없음</option>
                      {allProducts.map((p) => <option key={p.id} value={p.id}>{p.title}</option>)}
                    </select>
                    <input style={{ ...inputStyle, flex: 1, minWidth: 140 }} value={editDirForm.name}
                      onChange={(e) => setEditDirForm((f) => ({ ...f, name: e.target.value }))} placeholder="작가" />
                    <input style={{ ...inputStyle, flex: 1, minWidth: 140 }} value={editDirForm.instagram}
                      onChange={(e) => setEditDirForm((f) => ({ ...f, instagram: e.target.value }))} placeholder="인스타" />
                    <button style={btnStyle('#191919', '#fff')} onClick={() => handleUpdateDir(dir.id, dir.products[0]?.productId)}>저장</button>
                    <button style={btnStyle('#fff', '#666', '#ddd')} onClick={() => setEditDirId(null)}>취소</button>
                  </div>
                ) : (
                  <>
                    <button
                      onClick={() => handleExpandDir(dir.id)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, flex: 1, textAlign: 'left' }}
                    >
                      <span style={{ fontSize: 12, color: '#c9a96e', fontWeight: 700 }}>{dir.number}</span>
                      <span style={{ fontSize: 15, fontWeight: 600, color: '#1a1a1a' }}>{dir.name}</span>
                      {dir.instagram && <span style={{ fontSize: 12, color: '#888' }}>{dir.instagram}</span>}
                      <span style={{ fontSize: 11, color: '#aaa', marginLeft: 'auto' }}>
                        {packages.length > 0 ? `패키지 ${packages.length}개` : ''} {isExpanded ? '▲' : '▼'}
                      </span>
                    </button>
                    <div style={{ display: 'flex', gap: 6, marginLeft: 12 }}>
                      <button style={btnStyle('#fff', '#3a1a2a', '#ddd')} onClick={() => {
                        setEditDirId(dir.id);
                        setEditDirForm({ number: dir.number, name: dir.name, instagram: dir.instagram ?? '', productId: dir.products[0]?.productId ?? '' });
                      }}>수정</button>
                      <button style={btnStyle('#fff', '#e05555', '#fdd')} onClick={() => handleDeleteDir(dir.id)}>삭제</button>
                    </div>
                  </>
                )}
                </div>
              ...
              {/* 패키지 섹션 */}
              {isExpanded && (
                <div style={{ borderTop: '1px solid #ede8de', background: '#fafaf8', padding: '20px 20px 24px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    <p style={{ fontSize: 12, fontWeight: 700, color: '#3a1a2a', letterSpacing: '1px', textTransform: 'uppercase' }}>Packages</p>
                    {addingPkgForDir !== dir.id && (
                      <button style={btnStyle('#191919', '#fff')} onClick={() => {
                  ...
                  {addingPkgForDir === dir.id && (
                    <div style={{ marginBottom: 16 }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: '#3a1a2a', marginBottom: 10 }}>새 패키지</p>
                      <PackageForm
                        form={newPkgForm}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {packages.map((pkg) => (
                      <div key={pkg.id} style={{ background: '#fff', border: '1px solid #e8e0d0', borderRadius: 8, overflow: 'hidden' }}>
                        {editPkgId === pkg.id ? (
                          <div style={{ padding: 16 }}>
                            <p style={{ fontSize: 13, fontWeight: 600, color: '#3a1a2a', marginBottom: 10 }}>{pkg.name} 수정</p>
                            <PackageForm
                              form={editPkgForm}
                          ...
                          <div style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                <span style={{ fontSize: 14, fontWeight: 600, color: '#1a1a1a' }}>{pkg.name}</span>
                                {pkg.subtitle && <span style={{ fontSize: 12, color: '#888' }}>{pkg.subtitle}</span>}
                              </div>
                              ...
                              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 2 }}>
                                {pkg.partners.map(({ partner }) => (
                                  <span key={partner.id} style={{ fontSize: 11, background: '#f0e8d8', color: '#7a5520', borderRadius: 4, padding: '2px 6px' }}>
                                    {partner.role === 'hmu' ? 'HMU' : partner.role === 'dress' ? 'Dress' : 'Suit'}: {partner.name}
                                  </span>
                                ))}
                                {pkg.inclusions.length > 0 && (
                                  <span style={{ fontSize: 11, background: '#e8f0e8', color: '#2a5a2a', borderRadius: 4, padding: '2px 6px' }}>
                                    Inclusive {pkg.inclusions.length}개
                                  </span>
                                )}
                                {pkg.addons.length > 0 && (
                                  <span style={{ fontSize: 11, background: '#e8e8f0', color: '#2a2a5a', borderRadius: 4, padding: '2px 6px' }}>
                            ...
                            <div style={{ display: 'flex', gap: 6, marginLeft: 12, flexShrink: 0 }}>
                              ...
                              <button style={btnStyle('#fff', '#e05555', '#fdd')} onClick={() => handleDeletePkg(pkg.id, dir.id)}>삭제</button>
                            </div>
                          </div>
                        )}
                      </div>
                    ))}
                    {!pkgsLoading && packages.length === 0 && addingPkgForDir !== dir.id && (
