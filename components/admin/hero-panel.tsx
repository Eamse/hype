...
export default function HeroPanel({ brand }: {
    ...
    return (<div>
        ...
        {Array.from({ length: MAX_HERO }).map((_, i) => {
            ...
            return (<div key={i} style={{
              ...
              {url && typeof url === 'string' ? (<>
                  ...
                  {i < images.length || uploading ? (<div style={{
                            width: 18,
                            height: 18,
                            borderRadius: '50%',
      ...
      <input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif" multiple style={{ display: 'none' }} onChange={(e) => {
            if (e.target.files)
                handleFiles(e.target.files);
            e.target.value = '';
        }}/>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <button onClick={() => inputRef.current?.click()} disabled={uploading || images.length >= MAX_HERO} style={{
            padding: '10px 22px',
            background: '#000',
            color: '#fff',
            border: 'none',
