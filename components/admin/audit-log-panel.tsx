    ...
    return (<div>
      ...
      <div style={{
        ...
        <div>
          ...
          <input style={{ ...inputStyle, width: 220 }} value={q} placeholder="예: title, Jeju and You" onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && applyFilters()}/>
        </div>
        <div>
      ...
      {!loading && logs.length === 0 && (<p style={{ fontSize: 13, color: '#000' }}>기록된 로그가 없습니다.</p>)}
      ...
      {!loading && logs.length > 0 && (<>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {logs.map((log) => (<tr key={log.id} style={{ borderBottom: '1px solid #eee' }}>
          ...
          <div style={{
            ...
            <button style={btnStyle('#fff', '#000', '#000')} disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>
              이전
            </button>
            <span style={{ fontSize: 13, color: '#000' }}>
              {page} / {totalPages} ({total}건)
            </span>
            <button style={btnStyle('#fff', '#000', '#000')} disabled={page >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))}>
              다음
            </button>
          </div>
        </>)}
