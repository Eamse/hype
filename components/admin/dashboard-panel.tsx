...
export default function DashboardPanel({ onNavigation, isMobile, }: {
    ...
    const totalProducts = state?.product?.reduce((acc, p) => acc + p._count.id, 0) ?? 0;
    const photographerCount = state?.product
        ?.filter((p) => p.section.includes('Photographer'))
        .reduce((acc, p) => acc + p._count.id, 0) ?? 0;
    const casualCount = state?.product
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {!state ? (<div style={{ color: '#000', fontSize: 13, padding: '24px 0' }}>
          Loading...
        </div>) : (<>
          ...
          <div style={{
            ...
            <div style={{
              ...
              {gaError ? (<div style={{
                  ...
                  {([
                    ...
                    ['전체 누적', ga?.allTime],
                ] as const).map(([label, data]) => (<div key={label} style={{
                        display: 'flex',
                        alignItems: 'baseline',
              ...
              <div style={{
                ...
                alignItems: 'center',
                  ...
                  <input type="date" value={searchStart} onChange={(e) => setSearchStart(e.target.value)} style={{
                fontSize: 12,
                padding: '6px 8px',
                border: '1px solid #000',
                borderRadius: 6,
                color: '#000',
                  ...
                  <input type="date" value={searchEnd} onChange={(e) => setSearchEnd(e.target.value)} style={{
                fontSize: 12,
                padding: '6px 8px',
                border: '1px solid #000',
                borderRadius: 6,
                ...
                {searchError && (<p style={{ fontSize: 12, color: '#6a6a6a', marginTop: 10 }}>
                    조회 실패 — 기간을 확인해주세요
                  </p>)}

                {searchResult && !searchError && (<div style={{
                    marginTop: 12,
                    display: 'flex',
                    gap: 20,
                }}>
                    ...
                    <span>
                      <strong style={{ fontSize: 20, fontWeight: 800, color: '#000' }}>
            ...
            <div style={{
              ...
              {!state?.recentProducts?.length ? (<div style={{
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    height: 180,
                  ...
                  {state.recentProducts.map((p) => (<div key={p.id} onClick={() => onNavigation(p.section.includes('Casual')
                        ? 'Casual Photoshoot'
                        : 'Photographers')} style={{
                        display: 'flex',
                        alignItems: 'center',
                      ...
                      <div style={{
                        ...
                        {p.imageUrl && (<img src={p.imageUrl} alt="" style={{
                            width: '100%',
                            height: '100%',
                            objectFit: 'cover',
                        }}/>)}
          ...
          <div style={{
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {state?.product?.map((p, i) => (<tr key={p.section} onClick={() => onNavigation(p.section.includes('Casual')
                    ? 'Casual Photoshoot'
                    : 'Photographers')} style={{
                    ...
                    {!isMobile && (<td style={{ padding: '16px 24px', textAlign: 'right' }}>
                        <div style={{
                        display: 'flex',
                        alignItems: 'center',
    ...
    return (<div style={{ position: 'relative' }}>
      <svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height} style={{ display: 'block', overflow: 'visible' }}>
        ...
        {gridLevels.map((level) => {
            ...
            return (<g key={level}>
              <line x1={paddingLeft} y1={y} x2={width} y2={y} stroke="#eee" strokeDasharray={level === 0 ? undefined : '3 3'}/>
              <text x={paddingLeft - 8} y={y + 3} fontSize={9} textAnchor="end" fill="#999">
                {value}
              </text>
            </g>);
        })}
        ...
        <path d={linePath} fill="none" stroke="#7f7f7f" strokeWidth={2} strokeLinejoin="round" strokeLinecap="round"/>

        {hovered && (<line x1={hovered.x} y1={paddingTop} x2={hovered.x} y2={paddingTop + chartHeight} stroke="#ccc" strokeDasharray="3 3"/>)}

        {points.map(({ x, y, d }, i) => {
            const isHovered = hoverIndex === i;
            return (<g key={d.date}>
              <circle cx={x} cy={y} r={isHovered ? 4.5 : 3} fill="#7f7f7f" stroke="#fff" strokeWidth={isHovered ? 1.5 : 0}/>
              
              <circle cx={x} cy={y} r={10} fill="transparent" pointerEvents="all" style={{ cursor: 'pointer' }} onMouseEnter={() => setHoverIndex(i)} onMouseLeave={() => setHoverIndex((cur) => (cur === i ? null : cur))}/>
              <text x={x} y={height - 6} fontSize={9} textAnchor="middle" fill="#666">
