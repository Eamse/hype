...
function InstagramLink({ handle }) {
    ...
    return (<span>
      {handles.map((h, i) => (<span key={h}>
          <a href={`https://instagram.com/${h.replace("@", "")}`} target="_blank" rel="noopener noreferrer" style={{ color: "#1E5045", textDecoration: "none", borderBottom: "1px solid #5E9485" }}>
            {h}
          </a>
          {i < handles.length - 1 && " / "}
        </span>))}
...
function RenderDesc({ text }) {
    ...
    return parts.map((part, i) => part.startsWith("@") ? (<a key={i} href={`https://instagram.com/${part.replace("@", "")}`} target="_blank" rel="noopener noreferrer" style={{ color: "#1E5045", textDecoration: "none", borderBottom: "1px solid #5E9485" }}>{part}</a>) : <span key={i}>{part}</span>);
}
...
function RegionSelect({ onSelect, onBack }) {
    return (<div>
      ...
      <div style={{ display: "flex", gap: "16px", flexWrap: "wrap", justifyContent: "center" }}>
        ...
        ].map((r) => (<button key={r.key} onClick={() => onSelect(r.key)} style={{
                width: "160px",
                padding: "24px 20px",
                background: "#fff",
                border: "1px solid #5E9485",
    ...
    return (<div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "8px", maxWidth: "480px", margin: "0 auto" }}>
        {photographers.map((p) => {
            ...
            return (<ListButton key={p.id} onClick={() => available && onSelect(p)} disabled={!available} left={<>
                ...
                </>} right={available ? (<span style={{ color: "#555555", fontSize: "12px" }}>
                    {hasSubs ? `${p.subPhotographers.length} directors ›` : `${p.packages.length} packages ›`}
                  </span>) : (<span style={{ color: "#999", fontSize: "12px" }}>Coming Soon</span>)}/>);
        })}
      </div>
    </div>);
}
function SubPhotographerList({ photographer, onSelect, onBack }) {
    return (<div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "8px", maxWidth: "480px", margin: "0 auto" }}>
        {photographer.subPhotographers.map((sub) => (<ListButton key={sub.id} onClick={() => onSelect(sub)} disabled={false} left={<>
                <span style={{ color: "#555555", fontSize: "12px", fontWeight: 400 }}>{sub.number}</span>
                <span style={{ color: "#111111", fontSize: "16px", marginLeft: "12px", fontFamily: "'Pretendard', -apple-system, sans-serif", fontWeight: 400 }}>
                  {sub.name}
                </span>
              </>} right={<span style={{ color: "#555555", fontSize: "12px" }}>{sub.packages.length} packages ›</span>}/>))}
      </div>
    </div>);
}
function PackageDetail({ photographer, parentName, onBack, backLabel, selectedYear }) {
    ...
    return (<div>
      ...
      <div style={{ display: "flex", gap: "8px", justifyContent: "center", flexWrap: "wrap", marginBottom: "32px" }}>
        {currentPackages.map((p, i) => (<button key={p.name} onClick={() => { setSelectedPkg(i); setExpandedAddon(null); }} style={{
                padding: "10px 20px",
      ...
      <div style={{ background: "rgba(30, 80, 69, 0.06)", border: "1px solid #5E9485", borderRadius: "4px", padding: "20px 24px", marginBottom: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "12px" }}>
          {Object.entries(pkg.partners).map(([role, info]) => (<div key={role}>
      ...
      <div style={{ marginBottom: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "6px" }}>
          {pkg.inclusiveItems.filter(item => !item.startsWith("Note:")).map((item) => (<div key={item} style={{ fontSize: "14px", color: "#111111", padding: "6px 0", display: "flex", alignItems: "flex-start", gap: "8px" }}>
              <span style={{ color: "#555555", flexShrink: 0 }}>✓</span>
              {item}
            </div>))}
        </div>
        {pkg.inclusiveItems.filter(item => item.startsWith("Note:")).map((note) => (<p key={note} style={{ fontSize: "12px", color: "#555", fontStyle: "italic", margin: "12px 0 0", padding: "8px 12px", borderLeft: "2px solid #5E9485" }}>
            {note}
          </p>))}
        {pkg.inclusiveNotes && pkg.inclusiveNotes.map((note) => (<p key={note} style={{ fontSize: "12px", color: "#555", fontStyle: "italic", margin: "12px 0 0", padding: "8px 12px", borderLeft: "2px solid #5E9485" }}>
            {note}
          </p>))}
      </div>
      ...
      {pkg.priceNoSNS !== null ? (<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "24px" }}>
        ...
        </div>) : (<div style={{ background: "#1E5045", borderRadius: "4px", padding: "16px", textAlign: "center", marginBottom: "24px" }}>
      ...
      {pkg.priceNoSNS !== null && (<p style={{ fontSize: "12px", color: "#666", margin: "0 0 24px 4px" }}>
      ...
      <div style={{ marginBottom: "32px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
          {pkg.addons.map((addon, i) => (<div key={addon.name} style={{ border: "1px solid #5E9485", borderRadius: "4px", overflow: "hidden" }}>
              <button onClick={() => setExpandedAddon(expandedAddon === i ? null : i)} style={{
                width: "100%",
                display: "flex",
                justifyContent: "space-between",
                alignItems: "center",
              ...
              {expandedAddon === i && (<div style={{ padding: "0 16px 12px", fontSize: "13px", color: "#555", lineHeight: "1.6", background: "rgba(30, 80, 69, 0.06)" }}>
      ...
      <div style={{ textAlign: "center", padding: "32px 0", borderTop: "1px solid #5E9485" }}>
        ...
        <a href="https://wa.me/821062695990" target="_blank" rel="noopener noreferrer" style={{
...
export default function App() {
    ...
    return (<div style={{
      ...
      {showYear && (<div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "20px", marginTop: "60px" }}>
          <p style={{ color: "#666", fontSize: "14px", letterSpacing: "2px", textTransform: "uppercase", margin: 0 }}>
            When is your shoot year?
          </p>
          <div style={{ display: "flex", gap: "16px", flexWrap: "wrap", justifyContent: "center" }}>
            {[2026, 2027].map((year) => (<button key={year} onClick={() => setSelectedYear(year)} style={{
                    width: "160px",
                    padding: "24px 20px",
                    background: "#fff",
                    border: "1px solid #5E9485",
      ...
      {showList && (<PhotographerList region={region} photographers={PHOTOGRAPHERS[region]} onSelect={handlePhotographerSelect} onBack={() => setRegion(null)}/>)}
      {showSubs && (<SubPhotographerList photographer={photographer} onSelect={(s) => { setSubPhotographer(s); window.scrollTo(0, 0); }} onBack={() => setPhotographer(null)}/>)}
      {showPackages && (<PackageDetail photographer={subPhotographer} parentName={photographer.subPhotographers ? photographer.name : null} onBack={handleBack} backLabel={photographer.subPhotographers ? photographer.name : "Photographers"} selectedYear={selectedYear}/>)}
